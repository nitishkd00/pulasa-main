...
class UnifiedAuthService {
  ...
  async validateToken(token: string): Promise<ValidationResponse> {
    try {
      const response = await axios.post<ValidationResponse>(`${this.baseURL}/auth/validate`, {
        token
      });
  ...
  async getProfile(): Promise<{ success: boolean; user?: User; error?: string }> {
    try {
      const response = await axios.get<{ success: boolean; user: User }>(`${this.baseURL}/auth/profile`);
